...
// いわゆる純粋オブジェクト志向言語というのは、純粋ヒープ志向言語だと言える。
// ユーザーにスタック領域の操作を意識させずに、ヒープだけでプログラミングを完結させる。
// pythonだとCのconstがなかったりする、コンパイルを挟まないということは全てが実行時に確定するということで、
// イミュータブルなタプルオブジェクトでさえ、ヒープにメモリが確保されると考えられる。
...
fn work_with_reference() {
    ...
    // ポインタを通じてs1にはアクセスできる
    // 考察：s1は文字の実体に対する直接的なポインタを持っている
    // &s1はs1が指しているポインタを直接持っているわけではない
    // let s2 = s1の場合では、シャローコピーセマンティクスで解釈すると直接的ポインタがコピーされてしまう
    // つまりヒープオブジェクトに対する直接的ポインタが一つになるようにRustは積極的に解釈する
    // ムーブセマンティクスを優先し、ポインタのコピーを言語仕様レベルで抑制するというのがC++にない強みだと考えられる
