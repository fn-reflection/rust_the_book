...
// 所有権という概念はheapデータの管理するのに意味がある
// 値は所有者と呼ばれる変数がもつ。いかなる時も所有者は一つであり、所有者がスコープから外れた時に値は破棄される。

fn about_scope() {
    // sは有効ではない、まだ宣言されていない
    let s = "s have this"; // sは有効、 "s have this"はハードコードされた不変値(internされた値という奴か、スタック領域にある？)
    println!("{}", s); //sを参照できる。
} // 関数の終わり、sはスコープから外れる
...
fn calculate_length_without_ref(s: String) -> (String, usize) {
    let length = s.len();
    (s, length) // ムーブしたsをムーブし直さないと呼び出し元でsを使えない。端的な記述にならない。
}

fn work_with_reference() {
    let s1 = String::from("hello");
    let len = calculate_length_with_ref(&s1); // s1を指す参照(s1を指しているポインタと考えればいい)
...
fn work_with_mut_reference() {
    let mut s1 = String::from("hello"); // mutじゃないと文字列を変更できない。
    let s1_ref1 = &mut s1; // mutableな参照
