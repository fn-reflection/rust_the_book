...
impl User { // Userに紐づく関数、すなわちメソッドを実装
...
impl User { // implは複数回書いても問題ない、C#のpartial classっぽい、今回の事例では分けて書くメリットはない。トレイトなどの実装時に役立つ？
    fn test_account() -> User { // 関連関数、特定の構造体に紐づいた関数、Javaで言う所のスタティックメソッド、selfを使わないと関連関数
        User {
...
fn instantiate(){
    ...
    println!("{}", user.active);
    user.active=false; // mutなstructのプロパティは変更できる activeのみをmutとする記法はない。
    println!("{}", user.active);
}
